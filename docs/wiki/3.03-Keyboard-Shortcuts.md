...
- `R`: Toggle the in-progress tag on the currently focused task
...
- `1`–`9` / `0`: Set / clear the currently focused task's day order (see [[4.09-Task-Attributes]])
...
- Task movement shortcuts reorder all-day tasks within their day. Timed tasks keep their time order. A task with a day order moves by rank instead (see [[4.09-Task-Attributes]]).
