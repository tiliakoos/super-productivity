...
- **Day order** — An optional position (1, 2, 3, …) among the tasks planned for the same day, set from the task's context menu or by pressing a digit key on the focused task (`0` clears it). The badge shows the task's rank among that day's open ordered tasks, so completing one moves the others up and undoing it puts it back. Task lists can sort by it.
