...
- **Day order** — An optional position (1, 2, 3, …) among the top-level tasks planned for the same day, set from the task's context menu or by pressing a digit key on the focused task (`0` clears it). The badge shows the task's rank among that day's open ordered tasks, so completing one moves the others up and undoing it puts it back. The Today list, the Planner and the Week view place ordered tasks first, by rank; unordered tasks keep their manual order below them. Dragging an ordered task, or moving it with the move shortcuts, updates its rank.
